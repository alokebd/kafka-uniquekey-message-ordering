package com.java2tech.kafka.integration;
...
import com.java2tech.kafka.config.KafkaConfiguration;
...
import static com.java2tech.kafka.util.TestEventData.buildDemoInboundKey;
import static com.java2tech.kafka.util.TestEventData.buildDemoInboundPayload;
...
public class KafkaIntegrationTest {
    ...
    public static class KafkaTestListener {
        ...
        Map<OutboundKey, Set<Integer>> partitionsByKey = new HashMap<>();

        // Key to Partition / SeqNo
        Map<OutboundKey, List<ImmutablePair<Integer, Integer>>> messagesByKey = new HashMap<>();

        @KafkaListener(groupId = "KafkaIntegrationTest", topics = DEMO_OUTBOUND_TEST_TOPIC, autoStartup = "true")
        void receive(@Header(KafkaHeaders.RECEIVED_PARTITION) Integer partition, @Header(KafkaHeaders.RECEIVED_KEY) OutboundKey key, @Payload final OutboundPayload payload) {
    ...
    public void testSingleEvent() throws Exception {
        ...
        InboundKey inboundKey = buildDemoInboundKey(keyId);

        InboundPayload inboundPayload = buildDemoInboundPayload(1);
    ...
    public void testKeyOrdering() throws Exception {
        ...
        for (int i=0; i<totalMessages; i++) {
            Integer keyId = RandomUtils.nextInt(1, 6);
            InboundKey inboundKey = buildDemoInboundKey(keyId);
            InboundPayload inboundPayload = buildDemoInboundPayload(i+1);
